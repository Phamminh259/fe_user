...
import 'package:jobfe/features/job_card.dart';
import 'package:jobfe/screens/photo_viewer_screen.dart';
...
class _CompaniesDetailScreenState extends State<CompaniesDetailScreen>
  ...
  getCompanyDetail(int id) async {
    ...
    companyDetail = await CompanyApi.getCompanyDetail.sendRequest(
    ...
    if (userToken == null || userToken == '') {
      ...
      companyDetail = await CompanyApi.getCompanyDetailAuth.sendRequest(
        token: userToken,
        urlParam: '/${id.toString()}',
      );
    }
    setState(() {
      ...
      companyBg =
          companyDetail['background_image'] ?? 'https://i.pravatar.cc/200';
  ...
  followCompany(int id) async {
    ...
    if (userToken == null) {
      ...
      Map<String, String> jsonBody = {'company_id': id.toString()};
      var data = await CandidateCompanyApi.followCompany.sendRequest(
        token: userToken,
        body: jsonBody,
      );
      getCompanyDetail(widget.companyId);
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            runSpacing: 8,
                            children: [
                              for (int i = 0; i < lstCompanyImage.length; i++)
                                InkWell(
                                  onTap: () {
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
                                        builder:
                                            (ctx) => PhotoViewerScreen(
                                              initIdx: i,
                                              imageList: lstCompanyImage,
                                            ),
                                      ),
                                    );
                                  },
  ...
  return Container(
    ...
    child:
        openingJobs.isEmpty
            ...
            : Column(
              children: [
                for (int i = 0; i < openingJobs.length; i++)
                  JobCard(jobId: openingJobs[i]['id'], notifyParent: () {}),
              ],
            ),
  );
}
