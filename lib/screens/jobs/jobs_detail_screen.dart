...
import 'package:jobfe/apis/apis_list.dart';
import 'package:jobfe/common_widgets/body_text.dart';
import 'package:jobfe/common_widgets/bullet_list.dart';
import 'package:jobfe/common_widgets/header_text.dart';
import 'package:jobfe/modals/login_request.dart';
import 'package:jobfe/screens/companies/company_detail_screen.dart';
import 'package:jobfe/screens/my_cv/my_cv_screen.dart';
import 'package:jobfe/utils/date_utils.dart';
...
class _JobsDetailScreenState extends State<JobsDetailScreen>
  ...
  loginState() async {
    ...
    if (userToken != null && userToken != '') {
      Map<String, String> jsonBody = {'token': userToken};
  ...
  getJobDetail(int id) async {
    String? userToken = await storage.read(key: 'token');
    if (userToken == null || userToken == '') {
      jobDetail = await JobApi.getJobDetail.sendRequest(
        urlParam: '/${id.toString()}',
      );
    } else {
      jobDetail = await JobApi.getJobDetailAuth.sendRequest(
        token: userToken,
        urlParam: '/${id.toString()}',
      );
    }
    ...
    if (jobDetail.isNotEmpty) {
      setState(() {
        ...
        companyLogo =
            jobDetail['company']['logo_image'] ?? 'https://i.pravatar.cc/160';
        companyBg =
            jobDetail['company']['background_image'] ??
            'https://i.pravatar.cc/200';
        amount = jobDetail['amount'] ?? 0;
        expRequired = jobDetail['experience_required'] ?? '';
        salary =
            (jobDetail['salary_min'] != null
  ...
  getAllCV(int size, int page) async {
    String? userToken = await storage.read(key: 'token');
    if (userToken != null && userToken != '') {
      var data = await CandidateCVApi.getAllCVs.sendRequest(
        token: userToken,
        urlParam: '?size=$size&page=$page',
      );
      if (data != null) {
  ...
  applyJob() async {
    ...
    var data = await CandidateJobApi.applyJob.sendRequest(
      body: jsonBody,
      token: userToken,
    );
    if (data != null) {
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        if (userToken == null) {
                          ...
                          await CandidateJobApi.saveJob.sendRequest(
                            body: jsonBody2,
                            token: userToken,
                          );
                          getJobDetail(jobId);
                      ...
                      child: Row(
                        children: [
                          ...
                          const Icon(FluentIcons.bookmark_16_regular, size: 18),
                        ],
                      ),
                    ),
                                                                            ...
                                                                            (
                                                                              ...
                                                                              title: Container(
                                                                                ...
                                                                                child: Row(
                                                                                  ...
                                                                                  children: [
                                                                                    const Icon(
                                                                                      EneftyIcons.document_favorite_outline,
                                                                                      color:
                                                                                          Colors.red,
                                                                                      size:
                                                                                          35,
                                                                                    ...
                                                                                    Expanded(
                                                                                      child: Column(
                                                                                        ...
                                                                                        children: [
                                                                                          Text(
                                                                                            cvList[index]['name'].toString().toUpperCase(),
                                                                                          ...
                                                                                          Text(
                                                                                            'Upload date: ${cvList[index]['create_at'].toString().substring(0, 10)}',
                                                                                            style: const TextStyle(
                                                                                              fontSize:
                                                                                                  12,
                                                                                            ),
                    ...
                    children: <Widget>[
                      Container(
                        ...
                        child: TabBar(
                          ...
                          tabs: [myTab('Job Details', 0), myTab('Company', 0)],
                        ),
                      ),
                      Center(
                        child:
                  ...
                  onTap: () {
                    ...
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder:
                            (ctx) =>
                                CompaniesDetailScreen(companyId: companyId),
                      ),
                    );
                  },
                  child: const Text(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(color: const Color.fromARGB(255, 241, 242, 243), height: 8),
