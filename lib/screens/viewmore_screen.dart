...
class _ViewmoreScreenState extends State<ViewmoreScreen> {
  ...
  getData(int size, int page) async {
    ...
    switch (widget.dataType) {
      case 'Jobs fit you':
        data = await CandidateJobApi.getRecommendJobs.sendRequest(
          token: userToken,
      ...
      case 'Your viewed jobs':
        data = await CandidateJobApi.getViewedJobs.sendRequest(
          token: userToken,
          urlParam: '?size=$size&page=$page',
        );
        listData = data.map((e) => e).toList();
      ...
      case 'Your saved jobs':
        data = await CandidateJobApi.getSavedJobs.sendRequest(
          token: userToken,
          urlParam: '?size=$size&page=$page',
        );
        listData = data.map((e) => e).toList();
      ...
      case 'Popular Companies':
        ...
        data = await CompanyApi.filterCompany.sendRequest(
          body: jsonBody,
          urlParam: '?size=$size&page=$page',
        );
        listData = data.map((e) => e).toList();
                        ...
                        : ListView.builder(
                          ...
                          itemBuilder:
                              (BuildContext context, int index) =>
                                  widget.dataType != 'Popular Companies'
                                      ? JobCard(
                                        jobId: listData[index]['id'],
                                        notifyParent: () {},
                                      )
                                      : InkWell(
                                        onTap: () async {
