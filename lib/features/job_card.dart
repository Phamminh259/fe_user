...
import 'package:jobfe/screens/jobs/jobs_detail_screen.dart';
import 'package:jobfe/utils/date_utils.dart';

var storage = const FlutterSecureStorage();

class JobCard extends StatefulWidget {
  const JobCard({super.key, this.jobId, required this.notifyParent});
...
class _JobCardState extends State<JobCard> {
  ...
  getJobs(int id) async {
    ...
    if (userToken != null && userToken != '') {
      jobInfo = await JobApi.getJobDetailAuth.sendRequest(
        token: userToken,
        urlParam: '/${id.toString()}',
      );
    } else {
      jobInfo = await JobApi.getJobDetail.sendRequest(
        urlParam: '/${id.toString()}',
      );
    }

    if (jobInfo.isNotEmpty && mounted) {
      setState(() {
        jobId = jobInfo['id'] ?? 0;
        ...
        logoImage =
            jobInfo['company']['logo_image'] ?? 'https://i.pravatar.cc/40';
        province = jobInfo['company']['city_province']['name'] ?? 'Ho Chi Minh';
        salary =
            (jobInfo['salary_min'] ?? 'Negotiable') +
            (jobInfo['salary_max'] != null
                ? ' - ' + jobInfo['salary_max']
                : '');
        createdDate = AppDateUtils.daysBetween(
          jobInfo['start_date'] ??
        ...
        : Container(
          ...
          child: InkWell(
            onTap: () async {
              ...
              if (userToken != null && userToken != '') {
                Map<String, String> jsonBody = {'job_id': jobId.toString()};
                await CandidateJobApi.viewJob.sendRequest(
                  body: jsonBody,
                  token: userToken,
                );
                widget.notifyParent();
                      ...
                      children: [
                        ClipOval(
                          ...
                          child: SizedBox.fromSize(
                            ...
                            child: Image.network(logoImage, fit: BoxFit.cover),
                          ),
                        ),
